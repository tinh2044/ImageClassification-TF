...
def inception_res_a(x, scale, name):
    ...
    branch3x3_dbl = conv2d(branch3x3_dbl, 48, 3, padding="same", name=f"{name}.dbl_3x3_1")
    branch3x3_dbl = conv2d(branch3x3_dbl, 64, 3, padding="same", name=f"{name}.dbl_3x3_2")
...
def inception_res_b(x, scale, name):
    ...
    branch7x7 = conv2d(branch7x7, 160, (1, 7), padding="same", name=f"{name}.1x7")
    branch7x7 = conv2d(branch7x7,  192, (7, 1), padding="same", name=f"{name}.7x1")
...
def inception_res_c(x, scale, name):
    ...
    branch3x3 = conv2d(branch3x3, 224, (1, 3), padding="same", name=f"{name}.1x3")
    branch3x3 = conv2d(branch3x3, 256, (3, 1), padding="same", name=f"{name}.3x1")
...
def reduce_b(x, name):
    ...
    branch_2 = conv2d(branch_2, 288, 3, strides=2, name=f"{name}.branch2_3x3")

    branch_3 = conv2d(x, 256, 1, name=f"{name}.branch3_1x1")
    branch_3 = conv2d(branch_3, 288, 3, padding="same", name=f"{name}.branch3_3x3_1")
    branch_3 = conv2d(branch_3, 320, 3, strides=2, name=f"{name}.branch3_3x3_2")
...
def stem(x):
    ...
    x1 = layers.MaxPooling2D(3, strides=2)(x)
    x2 = conv2d(x, 96, 3, strides=2, name="conv_4")

    x = layers.Concatenate(name="concat_1")([x1, x2])

    x1 = conv2d(x, 64, 1, name="conv_5")
    x1 = conv2d(x1, 96, 3, name="conv_6")

    x2 = conv2d(x, 64, 1, name="conv_7")
    x2 = conv2d(x2, 64, (1, 7), padding="same" , name="conv_8")
    x2 = conv2d(x2, 64, (7, 1), padding="same", name="conv_9")
    x2 = conv2d(x2, 96, 3, name="conv_11")

    x = layers.Concatenate(name="concat_2")([x1, x2])

    x1 = conv2d(x, 192, 3, strides=2, name="conv_12")
    x2 = layers.MaxPooling2D(3, strides=2)(x)

    x = layers.Concatenate(name="concat_3")([x1, x2])

    return x


def InceptionResNetV2(num_cls, scale=0.3):
    ...
    for i in range(5):
        x = inception_res_c(x, scale, name=f"inception_res_c{i}")

    x = layers.GlobalAvgPool2D(name="avg_pool")(x)
    x = layers.Dense(num_cls, activation="softmax", name="output")(x)

    return keras.Model(inputs=_input, outputs=x, name="InceptionResNetV2")
