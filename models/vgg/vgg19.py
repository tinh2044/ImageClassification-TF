...
def VGG19(num_cls):
    ...
    x = layers.Conv2D(
        256, (3, 3), activation="relu", padding="same", name="block3.conv4"
    )(x)
    ...
    x = layers.Conv2D(
        512, (3, 3), activation="relu", padding="same", name="block4.conv4"
    )(x)
    ...
    x = layers.Conv2D(
        512, (3, 3), activation="relu", padding="same", name="block5.conv4"
    )(x)
    ...
    x = layers.Dense(num_cls, activation="softmax", name="predictions")(x)

    return keras.Model(inputs=_input, outputs=x, name="VGG19")
